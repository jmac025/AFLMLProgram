...
def get_player_data():
    ...
    get_data_query = "Select upper(player_name) as player_name, upper(venuename) as venuename, timeOnGroundPercentage, disposals, upper(opposition) as opposition, year(utcstarttime) as match_year from ML_Player_Data"
...
def train_disposals_model(data, cutoff_year, history_estimators, recent_estimators, testing: bool):
    ...
    X_hist = ct_hist.fit_transform(historical_data.drop(columns=['disposals']))
    y_hist = historical_data['disposals']
    ...
    X_hist_test = ct_hist.transform(historical_data.drop(columns=['disposals']))
    ...
    X_recent = ct_recent.fit_transform(recent_data.drop(columns=['disposals', 'venuename', 'opposition', 'match_year']))
    y_recent = recent_data['disposals']
    ...
    X_recent_test = ct_recent.transform(recent_data.drop(columns=['disposals', 'venuename', 'opposition', 'match_year']))
    ...
    if testing:
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        cursor.execute(insert_hist_query, history_estimators, 'Disposal History', mse_hist, mae_hist, rmse_hist, r2_hist, mape_hist, history_duration, datetime.now())
        conn.commit()
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        cursor.execute(insert_recent_query, recent_estimators, 'Disposal Recent', mse_recent, mae_recent, rmse_recent, r2_recent, mape_recent, recent_duration, datetime.now())
        conn.commit()
    else:
        ...
        with open('disposals_combined_model.pkl', 'wb') as f:
...
def get_player_details():
    ...
    # Retrieve player data
    get_data_query = """select distinct concat(upper(player_givenname), ' ', upper(player_surname)) as player_name
...
def test_models():
    # Train the model
    train_disposals_model(get_player_data(), 2024, 50, 50, True)
    train_disposals_model(get_player_data(), 2024, 100, 100, True)
    train_disposals_model(get_player_data(), 2024, 150, 150, True)
    train_disposals_model(get_player_data(), 2024, 200, 250, True)
    train_disposals_model(get_player_data(), 2024, 50, 300, True)

def train_model():
    train_disposals_model(get_player_data(), 2023, 200, 300, False)
        
def run_disposals_model():

    # Load the combined model and transformers from disk using pickle
    with open('disposals_combined_model.pkl', 'rb') as f:
        combined_model, ct_hist, ct_recent = pickle.load(f)

    player_data = get_player_details()

    # Extracting columns used for prediction
    hist_cols = ['player_name', 'venuename', 'opposition', 'timeOnGroundPercentage', 'match_year']
    recent_cols = ['player_name', 'timeOnGroundPercentage']
    ...
    for idx, row in enumerate(player_data.itertuples(index=False)):
        insert_query = """
        INSERT INTO PredictionData_Disposals (Player, Venue, Opponent, TimeOnGroundPercentage, Disposals, Load_date)
        VALUES (?, ?, ?, ?, ?, ?)
        """
        cursor.execute(insert_query, row.player_name, row.venuename, row.opposition, row.timeOnGroundPercentage, predictions[idx], formatted_result)
        conn.commit()
        
    print("Disposals run completed")